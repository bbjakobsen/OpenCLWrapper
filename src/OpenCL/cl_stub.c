...
*/
  //  path = "/vendor/lib64/egl/libq3dtools_adreno.so";
    if (!path) {
...
#if 0 // defined(__ANDROID__) || defined(ANDROID)
        pid_t pid = getpid();
                 ...
                 cl_event *        event)
{
    ...
    if (func) {
        return func(command_queue, num_svm_pointers, svm_pointers, pfn_free_func, user_data, num_events_in_wait_list, event_wait_list, event);
    }
    else {
        return CL_INVALID_PLATFORM;
    }
